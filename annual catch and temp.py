...
plt.title('annual mean catch vs mean temp at eMOLT site BN01')
ax1=fig.add_subplot(3,1,2)
ax1.plot(year_BD,c_BD,'--b',marker='*', markersize=10,linewidth=2)
ax1.set_xlabel('Year',fontsize=10)
ax1.set_ylabel('annual mean catch', color='b')
ax1.tick_params('y', colors='black')
ax2 = ax1.twinx()
ax2.plot(year_BD,t_BD, 'r-',linewidth=2)
ax2.set_ylabel('annual mean temperature', color='r')
ax2.tick_params('y', colors='black')
ax1.legend(['catch'],loc=2,fontsize=10)
ax2.legend(['temp'],loc=0,fontsize=10)
...
ax1.legend(['catch'],loc=2,fontsize=10)
ax2.legend(['temp'],loc=0,fontsize=10)
plt.savefig(save_dir+"annual mean catch and mean temp.png")
plt.show()      